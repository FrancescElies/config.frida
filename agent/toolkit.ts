...
const Analysis = {
  ...
  startMemoryTracking(suspectAfterMs = 5000): void {
    ...
    log(`[+] Memory tracking started (suspect after ${suspectAfterMs} ms)`);
  },
    ...
    if (suspectedLeaks.length > 0) {
      ...
      Object.entries(bySize)
        ...
        .forEach(([size, count]) => {
          ...
          log(`  ${count}x ${size} bytes (${(total / 1024).toFixed(2)} KB)`);
        });
    }
  },
  ...
  socketActivity(): void {
    ...
    Process.getModuleByName(lib)
      ...
      .filter(
        (ex) =>
          ex.type === "function" &&
          ["connect", "recv", "send", "read", "write"].some(
            (prefix) => ex.name.indexOf(prefix) === 0,
          ),
      )
      .forEach((ex) => {
  ...
  hookOpenSSL(): void {
    const targets = [
      ...
      { name: "EVP_DigestFinal_ex", args: ["ctx", "md", "size"] },
      {
        ...
        args: ["ctx", "cipher", "impl", "key", "iv", "enc"],
      },
