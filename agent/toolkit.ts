...
const Utilities = {
        ...
        readMem(
                ...
                try {
                        ...
                        log(`[!] Error reading memory: ${e.message}`);
                        return null;
                }
        },
        ...
        writeMem(
                ...
                try {
                        ...
                        log(`[+] Wrote ${type} at ${pointer}`);
                } catch (e: any) {
                        log(`[!] Error writing memory: ${e.message}`);
                }
        },
        ...
        allocate(
                ...
                try {
                        ...
                        log(`[!] Allocation error: ${e.message}`);
                        return null;
                }
        },
        ...
        hexDump(addr: NativePointer | string, size = 256): void {
                const pointer = typeof addr === "string" ? ptr(addr) : addr;
                log(hexdump(pointer, { length: size }));
        },
        ...
        callNative(
                ...
                try {
                        ...
                        log(`[+] ${fnName}() returned: ${result}`);
                        return result;
                } catch (e: any) {
                        log(`[!] Error calling native function: ${e.message}`);
                        return null;
                }
        },
...
const Hooking = {
        ...
        hookMalloc(verbose = true): void {
                ...
                this.hook("malloc", {
                        ...
                        onLeave(retval) {
                                ...
                                if (size) {
                                        Hooking.mallocAllocations.set(retval.toString(), {
                                                size,
                                                timestamp: Date.now(),
                                        });
                                }
                ...
                (globalThis as any).mallocTracker = {
                        count: () => Hooking.mallocAllocations.size,
                        total: () =>
                                Array.from(Hooking.mallocAllocations.values()).reduce(
                                        (sum, a) => sum + a.size,
                                        0,
                                ),
                        list: () =>
                                Array.from(Hooking.mallocAllocations.entries()).map(([addr, info]) => ({
                                        address: addr,
                                        size: info.size,
                ...
                log("[+] malloc tracker available as: mallocTracker");
        },
        ...
        hookFree(verbose = true): void {
                this.hook("free", {
                        ...
                        onEnter(args) {
                                ...
                                Hooking.mallocAllocations.delete(address);
                                if (verbose) log(`  freeing: ${args[0]}`);
                        },
                });
        },
        ...
        hookIO(): void {
                this.hook("read", {
                        ...
                        logReturn: false,
                        onEnter(args) {
                                log(`[READ] fd=${args[0]}, size=${args[2]}`);
                        },
                });
                ...
                this.hook("write", {
                        ...
                        logReturn: false,
                        onEnter(args) {
                                log(`[WRITE] fd=${args[0]}, size=${args[2]}`);
                        },
                });

                log("[+] IO hooks installed");
        },
...
const Aanalysis = {
        ...
        startMemoryTracking(suspectAfterMs = 5000): void {
                if (this.memoryListeners.length > 0) {
                        log("[!] Memory tracking already active");
                        return;
                }
        ...
        memoryReport(): void {
                const leaks = Array.from(Aanalysis.allocations.values()) as AllocationInfo[];
                const now = Date.now();
                ...
                log(`Unreleased Blocks      : ${leaks.length}`);
                log(`Suspected Leaks        : ${suspectedLeaks.length}`);
        ...
        analyzeMemory(duration = 10): void {
                ...
                log(`[*] Analyzing memory for ${duration} seconds...`);
        ...
        profile(functionName: string): void {
                if (this.profilerListeners.has(functionName)) {
                        log(`[!] Already profiling: ${functionName}`);
                        return;
                }
        ...
        reportPerformance(functionName: string | null = null): void {
                ...
                names.forEach((fn) => {
                        ...
                        log(`  Total Time : ${stats.totalTime} ms`);
        ...
        slowest(count = 5): void {
                ...
                sorted.slice(0, count).forEach(([fn, stats]) => {
                        ...
                        log(`${fn}: ${avg.toFixed(2)} ms avg (${stats.calls} calls)`);
                });
        },
};
                ...
                const listener = Interceptor.attach(addr, {
                        onEnter(args) {
                                if (maxCalls > 0 && count >= maxCalls) {
                                        listener.detach();
                                        Debugging.traceListeners.delete(fnName);
                                        log(`[*] Max traces (${maxCalls}) reached, stopping`);
                                        return;
                                }
                                ...
                                log("  Backtrace:");
                                symbols.forEach((symbol, i) => log(`    [${i}] ${symbol}`));
                        },
                });
...
const Network = {
        ...
        hookAPI(
                ...
                functionNames.forEach((fnName) => {
                        ...
                        const listener = Interceptor.attach(addr, {
                                onEnter(args) {
                                        Network.callCount++;
                                        log(`[${Network.callCount}] ${fnName} called`);
        ...
        recentCalls(count = 10): void {
                ...
                this.calls.slice(-count).forEach((call) => {
                        log(`[${call.id}] ${call.name}`);
                        log(`  Time: ${new Date(call.time).toISOString()}`);
                        log(`  Args: ${call.args.join(", ")}`);
                });
        },
        ...
        apiStats(): void {
                ...
                log("\n=== API STATISTICS ===");
                log(`Total Calls: ${this.callCount}`);
                log("\nBy function:");
                                ...
                                onEnter(args: InvocationArguments) {
                                        try {
                                                ...
                                                patterns.forEach((pattern) => {
                                                        ...
                                                        log(`[ALERT] Suspicious data detected: ${pattern}`);
                                                        log(`  Data: ${data.substring(0, 120)}`);

                                                        Network.detectedData.push({
                                                                pattern,
...
const Crypto = {
        hookOpenSSL(): void {
                const targets = [
                        ...
                        { name: "EVP_DigestUpdate", args: ["ctx", "data", "count"] },
                        { name: "EVP_DigestFinal_ex", args: ["ctx", "md", "size"] },
                        { name: "EVP_CipherInit_ex", args: ["ctx", "cipher", "impl", "key", "iv", "enc"] },
                        { name: "EVP_CipherUpdate", args: ["ctx", "out", "outl", "in", "inl"] },
                        { name: "EVP_CipherFinal_ex", args: ["ctx", "out", "outl"] },
                        { name: "AES_encrypt", args: ["in", "out", "key"] },
                        { name: "AES_decrypt", args: ["in", "out", "key"] },
                        { name: "RSA_public_encrypt", args: ["flen", "from", "to", "rsa", "padding"] },
                        { name: "RSA_private_decrypt", args: ["flen", "from", "to", "rsa", "padding"] },
                ];
        ...
        hookCommonCrypto(): void {
                // macOS/iOS specific
                ["CC_SHA1", "CC_SHA256", "CC_MD5", "CCCryptorCreate", "CCCryptorUpdate", "CCCryptorFinal"].forEach(fn => {
                        Hooking.hook(fn, { logReturn: false });
                });
        },

        hookBCrypt(): void {
                // Windows
                ["BCryptEncrypt", "BCryptDecrypt", "BCryptHashData", "BCryptFinishHash"].forEach(fn => {
                        Hooking.hook(fn, { logReturn: false });
                });
        },
};
